    ...
    return (
            <main className="flex flex-col items-center
                    ...
                    <article className="flex flex-col items-center mt-20 mx-2 mb-6 p-4 h-fit
                                     bg-blue-900 border-2 rounded-lg w-5/6 max-w-5xl
                        ...
                        <div className={`mt-2 sm:mt-0 sm:ml-2 lg:max-w-lg`}>
                            ...
                            <p className="bg-blue-900 rounded-lg p-1 mt-2 text-justify lg:text-lg xl:text-xl">
                                {(() => {
                                    if(description === "") {
                                        return `${'> '}Description not available (but it's an amazing book!)`;
                                    } else {
                                        return (<>
                                            {isReadMoreShown ? `${'> '}${description.substr(0, 100)}` : `${'> '}${description}`} 
                                            <button onClick={toggleReadBtn} className="text-blue-300 ml-1">{isReadMoreShown? "... Read more" : "Read less"}</button>
                                        </>)
                                    }
                                })()}
                            ...
                            <div className="my-2 lg:text-lg xl:text-xl">
                                ...
                                <ul className={`list-inside list-disc bg-blue-800 rounded-lg p-1 mt-1 ${showLinks ? 'block': 'hidden'}`}>
